...
if( "NULL" != "NULL" ){
    # read the file
    tmp <- read.table("NULL", header=FALSE, as.is=TRUE)
...
# R object (AR model)
save(AR, file=paste("first_result/result.txt", "_AR.RData", sep=""))

# x-fold Cross-validation
CV <- cv.park(AR$OX, AR$OY, FOLD, 10)
# R object (CV data)
save(CV, file=paste("first_result/result.txt", "_CV.RData", sep=""))

write.table(paste("#After Try CV: ", "LC= ", round(CV$LC,keta), " TC= ", round(CV$TC,keta), sep=""),
            file=out,sep="",quote=FALSE,append=TRUE, row.names=FALSE, col.names=FALSE)
#===================================================#

# Residual: first_result/result.txt.residual.txt is created
...
write.table( temp, file="first_result/result.txt.residual.txt", sep="\t", quote=FALSE, append=FALSE, row.names=FALSE,col.names=TRUE)
...
#--- Plots ----------------------#
postscript("first_result/result.txt.ps")
...
for( i in 1:length(zthres)){
    cut <- zthres[i]
    idx <- abs(zscore) >= cut
    filename <- paste0("first_result/result.txt.outlier", "_" , zid[i] , ".txt")
