...
if($SKIP_GENEFILE ne "NULL"){
	...
	undef(@data);
	@data = `grep -v "\#3" $SKIP_GENEFILE | cut -f 1 | uniq `;
	chop(@data);
	if(scalar(@data) < 1){
	    $SKIP_GENEFILE = "NULL";
	}
...
sub RUN_GLMGE{
    ...
    my $outlier_file = $outfile . ".outlier";
    my $min_features = 10;
...
write.table( temp, file="$resid_file", sep="\\t", quote=FALSE, append=FALSE, row.names=FALSE,col.names=TRUE)
...
for( i in 1:length(zthres)){
    cut <- zthres[i]
    idx <- abs(zscore) >= cut
    filename <- paste0("$outlier_file", "_" , zid[i] , ".txt")
